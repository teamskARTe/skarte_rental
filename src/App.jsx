...
import { track, trackPageView, toGaItem, setAnalyticsUser } from './lib/analytics';
...
export function App() {
  ...
  // ── GA4: 라우트별 제목 설정 + 페이지뷰 전송 ──
  useEffect(() => {
    const PAGE_TITLES = {
      home: 'skARTe Rental — 인천 카메라·시네마 장비 대여',
      gear: '장비 대여 — skARTe Rental',
      guide: '이용 안내 — skARTe Rental',
      extra: '기타 장비 문의 — skARTe Rental',
      lookup: '문의 조회 — skARTe Rental',
      location: '오시는 길 — skARTe Rental',
      mypage: '마이페이지 — skARTe Rental',
      admin: '관리자 — skARTe Rental',
    };
    document.title = PAGE_TITLES[page] || PAGE_TITLES.home;
    trackPageView(location.pathname, document.title);
  }, [page]);

  // ── GA4: 장비 상세 조회 (view_item) ──
  useEffect(() => {
    if (!selectedItem) return;
    track('view_item', {
      currency: 'KRW',
      value: selectedItem.price || 0,
      items: [toGaItem(selectedItem)],
    });
  }, [selectedItem]);

  // ── GA4: 장비 검색 (0.8초 디바운스) ──
  useEffect(() => {
    const q = gearSearch.trim();
    if (q.length < 2) return;
    const t = setTimeout(() => track('search', { search_term: q }), 800);
  ...
  const addToCart = (item) => {
    ...
    track('add_to_cart', { currency: 'KRW', value: item.price || 0, items: [toGaItem(item)] });
  ...
  const logout = () => { setUser(null); setPage('home'); setAnalyticsUser(null); showToast('로그아웃되었습니다.'); };
  ...
  const toggleWish = (id) => {
    ...
    setWishlist(prev => {
      ...
      if (adding) {
        const g = equipment.find(e => e.id === id);
        if (g) track('add_to_wishlist', { currency: 'KRW', value: g.price || 0, items: [toGaItem(g)] });
      }
  ...
  const recordOrder = ({ items, total, startDate, type = 'cart', gear, situation, contact, name }) => {
    ...
    // ── GA4: 핵심 전환 이벤트 (대여 문의 접수) ──
    track('generate_lead', {
      currency: 'KRW',
      value: o.total || 0,
      lead_type: o.type,           // 'cart' | 'extra'
      ref_no: o.refNo,
      items: (o.items || []).map(it => ({ item_id: it.id, item_name: it.name, quantity: it.qty })),
    });
    return o;
  };
