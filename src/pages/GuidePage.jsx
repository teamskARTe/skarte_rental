...
export function GuidePage({ setPage }) {
  const steps = [
    ...
    { n:'05', t:'수령·반납', d:'송도/인하대 지점에 방문해 수령하고, 대여 종료일에 방문 반납합니다. 픽업 지점과 반납 지점은 장바구니에서 각각 선택하실 수 있어요. 정상 렌탈·반납 시 예약금은 전액 환불됩니다.' },
  ];
  const policies = [
  ...
  const faqs = [
    ...
    { q:'수령과 반납 방법은요?', a:'송도 지점(연수구 송도미래로 30)과 인하대 지점(미추홀구 인하로 100) 중에서 선택해 방문 수령하실 수 있습니다. 픽업 지점과 반납 지점을 서로 다르게 지정하는 것도 가능합니다. 택배 발송을 원하시면 카카오톡으로 문의해 주세요(왕복 배송비 별도).' },
